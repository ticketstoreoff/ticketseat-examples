...
function buildTicketSeatSnippet(method, path, body) {
  const lines = [
    `# Cote serveur Flask : le token reste dans .env`,
    `response = requests.${method.toLowerCase()}(`,
    `    f"{TICKET_SEAT_API_BASE_URL}${path}",`,
    `    headers={"Authorization": f"Bearer {TICKET_SEAT_PARTNER_TOKEN}"},`,
  ];
  if (body) {
...
// Appel depuis la page demo vers Flask
${buildLocalApiSnippet(localMethod, localPath, body)}

// Appel equivalent execute par Flask vers Ticket Seat
