...
function buildTicketSeatSnippet(method, path, body) {
  const lines = [
    `// Cote serveur PHP : le token reste dans .env`,
    `$curl = curl_init($ticketSeatApiBaseUrl . "${path}");`,
    `curl_setopt($curl, CURLOPT_CUSTOMREQUEST, "${method}");`,
    `curl_setopt($curl, CURLOPT_HTTPHEADER, ["Authorization: Bearer " . $partnerToken]);`,
  ];
  if (body) {
    lines.push(`curl_setopt($curl, CURLOPT_POSTFIELDS, json_encode(${JSON.stringify(body, null, 2)}));`);
  }
  lines.push("$response = curl_exec($curl);", "echo $response;");
  return lines.join("\n");
}
...
// Appel depuis la page demo vers PHP
${buildLocalApiSnippet(localMethod, localPath, body)}

// Appel equivalent execute par PHP vers Ticket Seat
