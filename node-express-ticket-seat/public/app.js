...
function buildTicketSeatSnippet(method, path, body) {
  const lines = [
    `// Cote serveur Express : le token reste dans .env`,
    `const response = await fetch(\`\${TICKET_SEAT_API_BASE_URL}${path}\`, {`,
    `  method: "${method}",`,
    `  headers: { Authorization: \`Bearer \${TICKET_SEAT_PARTNER_TOKEN}\` },`,
  ];
  if (body) {
    lines.push(`  body: JSON.stringify(${JSON.stringify(body, null, 2).replace(/\n/g, "\n  ")}),`);
  }
...
// Appel depuis la page demo vers Express
${buildLocalApiSnippet(localMethod, localPath, body)}

// Appel equivalent execute par Express vers Ticket Seat
